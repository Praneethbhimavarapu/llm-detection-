# Full Python code with Perceptual Hashing and modified output logic for both text and images
...
class MultiModalMemorizationDetector:
    ...
    def _detect_generic_similarity(self, content: str, content_hash: str, content_type: str) -> MemorizationResult:
        if content_hash in self.content_hashes.get(content_type, set()):
             ...
             return MemorizationResult(content_hash, 1.0, True, 1.0, str(item['content']), item['source'], "exact_hash_match", content_type)
        ...
        for item in self.training_data[content_type]:
            ...
            if similarity > best_similarity: best_similarity, best_match = similarity, item
        is_memorized = best_similarity >= self.threshold
        return MemorizationResult(content_hash, best_similarity, is_memorized, best_similarity if is_memorized else (1.0 - best_similarity), str(best_match['content']) if best_match else None, best_match['source'] if best_match else None, "ngram_jaccard_similarity", content_type)
